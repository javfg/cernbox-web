<template>
  <oc-modal :title="$gettext('Notification Settings')" :button-cancel-text="$gettext('Discard')"
    :button-confirm-text="$gettext('Save')" @cancel="$emit('cancel')" @confirm="onConfirm">
    <template #content>
      <oc-checkbox v-model="localNotifyEnabled" label="Disable all notifications" />
    </template>
  </oc-modal>
</template>
