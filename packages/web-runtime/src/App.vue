...
export default defineComponent({
  ...
  computed: {
    ...
    ...mapGetters(['configuration', 'capabilities', 'getSettingsValue', 'toursCurrentLanguage']),
    layout() {
  ...
  watch: {
    $route: {
      ...
      handler: function (to) {
        ...
        if (this.toursCurrentLanguage.length > 0) autostartTours(this.toursCurrentLanguage, to.name)
      }
    },
    capabilities: {
      immediate: true,
    ...
    selectedLanguage: {
      ...
      handler(language) {
        ...
        if (languageCode) {
          ...
          if (this.toursCurrentLanguage.length > 0) this.setToursCurrentLanguage()
        }
      }
    }
  },
  ...
  methods: {
    ...mapActions(['fetchNotifications', 'setToursCurrentLanguage']),
